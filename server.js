...
function pipeResponse(res, fromRes) {
  var h = {};
...
function proxyRequest(res, targetUrl, method, headers, body) {
...
function serveStaticFile(res, reqUrl) {
...
function handler(req, res) {
  ...
  if (req.url.indexOf('/api/doubao/') === 0) {
    ...
    collectBody(req, function(body) { proxyRequest(res, target, 'POST', hdrs, body) });
    return;
  }
  ...
  if (req.url.indexOf('/api/bailian/') === 0) {
    ...
    collectBody(req, function(body) { proxyRequest(res, target2, 'POST', hdrs2, body) });
    return;
  }
  ...
  if (req.url.indexOf('/api/fangzhou/') === 0) {
    ...
    collectBody(req, function(body) { proxyRequest(res, target3, 'POST', hdrs3, body) });
    return;
  }
  ...
  if (req.url.indexOf('/api/proxy-url') === 0) {
    ...
    proxyRequest(res, extUrl, 'GET', { 'Host': extParsed.hostname }, null);
    return;
  }
